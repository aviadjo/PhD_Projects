 ...
 * @author Aviad
 */
public class FeatureExtractorNgramsChar<T> extends AFeatureExtractor<T> {

    private final int m_grams; //the gram size to be used    
    private final int m_skip; //the skip to be used

    public FeatureExtractorNgramsChar(int gram_size, int skip) {
        m_grams = gram_size;
        m_skip = skip;
    }
    ...
    public String GetName() {
        return String.format("char n-gram (grams=%s skip=%s)", m_grams, m_skip);
    }
