...
import Feature_Selection.Abstract_Feature_Selector;
 ...
 * @author Aviadjo
 */
public class FS_InfoGainRatio extends Abstract_Feature_Selector {
     ...
     * Class B
     */
    public FS_InfoGainRatio(int Class_A_elements_num, int Class_B_elements_num, boolean GainRatio) {
        ...
        m_Total_entropy = Math_Calc.Get_Entropy(new ArrayList<>(Arrays.asList(m_Class_A_elements_percentage, m_Class_B_elements_percentage)));
    }
    ...
    public HTreeMap<String, Double> Get_Features_InfoGain(HTreeMap<String, int[]> features_DFs) {
        ...
        for (Map.Entry<String, int[]> entry : features_DFs.entrySet()) {
            ...
            class_A_P = Math_Calc.Get_P_with_Laplace_Correction(class_A, class_A + class_B, 2);
            class_B_P = Math_Calc.Get_P_with_Laplace_Correction(class_B, class_A + class_B, 2);
            entropy = Math_Calc.Get_Entropy(new ArrayList<>(Arrays.asList(class_A_P, class_B_P)));
            //*************************************************
            class_Ax = m_Class_A_elements_num - class_A;
            class_Bx = m_Class_B_elements_num - class_B;
            class_Ax_P = Math_Calc.Get_P_with_Laplace_Correction(class_Ax, class_Ax + class_Bx, 2);
            class_Bx_P = Math_Calc.Get_P_with_Laplace_Correction(class_Bx, class_Ax + class_Bx, 2);
            entropyx = Math_Calc.Get_Entropy(new ArrayList<>(Arrays.asList(class_Ax_P, class_Bx_P)));
            //*************************************************
